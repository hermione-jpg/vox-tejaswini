...
import {
  getVoiceAgents,
  getVoiceAgent,
  getVoiceAgentContent,
  getVoiceAgentSlugs,
} from "@/lib/getVoiceAgents";
...
export async function generateStaticParams() {
  const slugs = await getVoiceAgentSlugs();
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps) {
  const { slug } = await params;
  const lesson = await getVoiceAgent(slug);
...
export default async function LessonPage({ params }: PageProps) {
  ...
  const source = await getVoiceAgentContent(slug);
  if (!source) notFound();
      ...
      <div className="flex-1 bg-white">
        <Container className="py-16">
          <article className="max-w-3xl text-[#2F3E46]">
            <h1 className="text-[40px] leading-[44px] font-semibold tracking-tight">
              {lesson.title}
            </h1>
            ...
            {lesson.next && (
              <div className="mt-20 border-t border-[#9A9A9A] pt-8">
                <Link
                  href={`/voice-agents/${lesson.next.slug}`}
